...
# ── 流式管道 ─────────────────────────────────────────────
async def generate_stream(prompt: str, api_key: str, api_url: str, model_name: str, user_id: str, db: Session):
    """把 LLM 的原始 SSE 块过滤为纯文本 data: 行，推给前端，并保存聊天历史。"""
    full_ai_reply = ""
    try:
        async for chunk in call_llm_stream(prompt, api_key, api_url, model_name):
...
async def chat(
    ...
    return StreamingResponse(
        generate_stream(data.message, config.api_key, config.api_url, config.model_name, data.user_id, db),
